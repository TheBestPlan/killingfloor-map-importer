...
// Tactical Ops and Unreal Tournament 99: an Unreal Engine 1 client and a map name inside it - the same
// shape as Quake 3. One converter, job.profile ("to" | "ut99") picks the install layout.
function convertTO(job, log) {
  const to = require("../src/tacticalops/convert");
  const res = to.convert({
    profile: job.profile || job.game || "to",
    clientDir: job.clientDir, map: job.map, mapFile: job.mapFile || null, mapName: job.name || null,
    outDir: job.outDir || null,
...
// Source 2 (CS2) map: a .vpk whose world geometry is baked into embedded meshes (KV3 + meshopt).
function convertSource2(job, log) {
  const s2 = require("../src/source2/convert");
  const res = s2.convert({
    file: job.bspFile, mapName: job.name || null, outDir: job.outDir || null,
    scale: job.sourceScale, emitPlayerStarts: job.emitPlayerStarts !== false,
    ambient: job.ambient, glow: job.glow, texGain: job.texGain, cullDistance: job.cullDistance,
    autoColor: job.autoColor !== false, log,
  });
  const v = verify(res.out);
  for (const line of v.report.split("\n")) log(line);
  process.send({
    kind: "done", ok: v.ok, out: res.out, mapName: res.mapName, size: fs.statSync(res.out).size,
    nodes: 0, surfs: 0, lightMaps: 0, atlases: 0, textures: 0, missingTextures: 0,
    mesh: { triangles: res.stats ? res.stats.triangles : 0, meshes: res.meshes },
  });
}

// Project IGI: a level's models .res (ILFF) of world-space .mef meshes.
function convertIgi(job, log) {
  const igi = require("../src/igi/convert");
  const res = igi.convert({
    file: job.bspFile, mapName: job.name || null, outDir: job.outDir || null,
    ambient: job.ambient, glow: job.glow, autoColor: job.autoColor !== false,
    cullDistance: job.cullDistance, emitPlayerStarts: job.emitPlayerStarts !== false, log,
  });
  const v = verify(res.out);
  for (const line of v.report.split("\n")) log(line);
  process.send({
...
// Vampire: The Masquerade - Redemption: a .nil level (or Levels.nob archive) - Nod scene meshes.
function convertVtmr(job, log) {
  const vtmr = require("../src/vtmr/convert");
  const res = vtmr.convert({
    file: job.bspFile, level: job.level || null, mapName: job.name || null, outDir: job.outDir || null,
    ambient: job.ambient, glow: job.glow, autoColor: job.autoColor !== false,
    cullDistance: job.cullDistance, emitPlayerStarts: job.emitPlayerStarts !== false, log,
  });
  const v = verify(res.out);
...
// Savage: a .s2z map archive - its terrain heightfield becomes a KF landscape.
function convertSavage(job, log) {
  const sv = require("../src/savage/convert");
  const res = sv.convert({
    file: job.bspFile, mapName: job.name || null, outDir: job.outDir || null,
    ambient: job.ambient, glow: job.glow, autoColor: job.autoColor !== false,
    emitPlayerStarts: job.emitPlayerStarts !== false, log,
  });
  const v = verify(res.out);
  for (const line of v.report.split("\n")) log(line);
  process.send({
...
// GTA III / Vice City: a RenderWare map, from the game's install root; one district is cropped out.
function convertGta(job, log) {
  const gta = require("../src/gta/convert");
  const res = gta.convert({
    clientDir: job.clientDir, game: job.game === "vc" ? "vc" : "gta3", mapName: job.name || null, outDir: job.outDir || null,
    scale: job.gtaScale, crop: job.crop || null, whole: job.whole === true,
    ambient: job.ambient, glow: job.glow, texGain: job.texGain, cullDistance: job.cullDistance,
    emitPlayerStarts: job.emitPlayerStarts !== false, log,
  });
  const v = verify(res.out);
...
const GAME_CONVERTERS = { l2: convertL2, q3: convertQ3, to: convertTO, ut99: convertTO, source: convertSource, source2: convertSource2, gta3: convertGta, vc: convertGta, savage: convertSavage, igi: convertIgi, vtmr: convertVtmr, model: convertModel };

...
function buildMeshes(map, opts) {
  ...
  for (const ent of map.entities) {
    ...
    jobs.push({
      ...
      // `rendermode 4` (Solid) is the one that turns a `{` texture's colour key transparent. On
      // anything else - worldspawn included - GoldSrc draws the key colour, so neither may this.
      cutout: brushEnts.cutsOut(ent),
  ...
  for (const job of jobs) {
    for (let fi = job.model.firstface; fi < job.model.firstface + job.model.numfaces; fi++) {
      ...
      // A cut-out is a property of the ENTITY, not of the texture: see the note where the jobs are
      // built. Without one asking for it the face wears the solid copy of the picture.
      let matRef = (job.mat && job.mat(tex)) || 0;
      if (!matRef) {
        matRef = tex.ref;
        if (tex.kind === "masked" && !job.cutout && opts.solidMasked) {
          matRef = opts.solidMasked(ti.miptex) || tex.ref;
          if (matRef !== tex.ref) stats.uncut = (stats.uncut || 0) + 1;
        }
      }

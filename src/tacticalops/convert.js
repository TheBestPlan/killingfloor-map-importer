...
const { installedTacticalOps, installedUT99 } = require("../resources");

const manifest = require("../../package.json");
const TOOL_NAME = manifest.productName;
const TOOL_URL = manifest.repository.url.replace(/^git\+/, "").replace(/\.git$/, "");

// One converter, two Unreal Engine 1 games. Tactical Ops is a UT99 total conversion, so its packages
// and UT99's are byte-for-byte the same container (tag 0x9E2A83C1) with the same tagged properties,
// world axes and pawn (UT99's TournamentPlayer) - only the install layout and the map-name prefixes
// differ. A profile carries those: the mod-subfolder roots to index, which filenames are playable
// maps, how far up from a loose .unr the client root sits, and the game title stamped into the .rom.
const PROFILES = {
  to: {
    gameName: "Tactical Ops: Assault on Terror",
    roots: ["TacticalOps", "."], mapUp: 2, installed: installedTacticalOps,
    mapFilter: (key) => /^to[-_]/i.test(key),
  },
  ut99: {
    gameName: "Unreal Tournament",
    roots: ["."], mapUp: 1, installed: installedUT99,
    // The shipped gametype prefixes; the menu backdrops (Entry, CityIntro, UTCredits, UT-Logo-Map) are
    // not playable and stay out, the same way TO drops its own Entry/Intro.
    mapFilter: (key) => /^(dm|dmjp|ctf|dom|as|eol)[-_]/i.test(key),
  },
};
...
function findMap(clientDir, name, clientOpts) {
  const client = new TO.Client(clientDir, clientOpts);
...
function convert(opts) {
  ...
  const prof = PROFILES[o.profile] || PROFILES.to;
  const GAME = prof.gameName;
  const clientOpts = { roots: prof.roots, mapFilter: prof.mapFilter };

  // Either a loose .unr, or a map name inside an installed client.
  let client = null, file = null, baseName = null;
  const clientDir = o.clientDir || prof.installed()[0] || null;
  if (o.mapFile) {
    file = o.mapFile;
    baseName = path.basename(file).replace(/\.unr$/i, "");
    // The map still needs the client for its textures: an Unreal Engine 1 map holds almost none of its
    // own, and without the .utx packages every surface would be a placeholder. A loose .unr sits
    // mapUp folders below the client root (TO: root/TacticalOps/Maps, UT99: root/Maps).
    const up = Array(prof.mapUp).fill("..");
    client = new TO.Client(clientDir || path.resolve(path.dirname(file), ...up), clientOpts);
  } else {
    if (!clientDir || !o.map) throw new Error("give either a .unr file or --client <folder> --map <name>");
    const found = findMap(clientDir, o.map, clientOpts);
...
module.exports = { convert, DEFAULTS, PROFILES };

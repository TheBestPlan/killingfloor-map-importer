...
// The mod's own tree first, then the UT99 install it sits in. A plain Unreal/UT99 install has no mod
// subfolder, so the ut99 profile passes roots: ["."] instead.
const ROOTS = ["TacticalOps", "."];
const FOLDERS = ["Maps", "Textures", "System", "Sounds", "Music", "StaticMeshes"];
// The game's own playable maps, by filename prefix: TO-* for Tactical Ops. The ut99 profile widens
// this to the UT gametype prefixes.
const TO_MAP_FILTER = (key) => /^to[-_]/i.test(key);
...
class Client {
  // opts.roots: subfolders under `root` to index, best first (default the TO mod tree). opts.mapFilter:
  // (key) -> is this a playable map (default TO's TO-* prefix). The UT99 route passes its own.
  constructor(root, opts) {
    opts = opts || {};
    this.root = root;
    this.roots = opts.roots || ROOTS;
    this.mapFilter = opts.mapFilter || TO_MAP_FILTER;
    this.byName = new Map();
    this.open = new Map();
    for (const sub of this.roots) {
  ...
  maps() {
    ...
    for (const [key, file] of this.byName) {
      ...
      if (!this.mapFilter(key, file)) continue;

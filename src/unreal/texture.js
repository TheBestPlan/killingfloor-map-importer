...
// binaryAlpha: for a STY_Masked cut-out (foliage, fences), the alpha is a hard 0/255 edge and must STAY
// binary down the mip chain - averaging it (as the plain path does) leaves lower mips semi/fully opaque, so
// a distant tree card loses its cut-out and draws as a solid rectangle. Downsample the cut-out by COVERAGE:
// a mip texel is opaque only if >=half of its four source texels are, and its colour is the average of just
// the opaque ones (so the transparent border does not bleed into the leaf edge).
function mipChain(px, w, h, coverage, binaryAlpha) {
  ...
  while (cur.width > 1 || cur.height > 1) {
    ...
    for (let y = 0; y < nh; y++) {
      for (let x = 0; x < nw; x++) {
        ...
        if (binaryAlpha) {
          const cov = at.filter((i) => cur.data[i * 4 + 3] >= 128);
          const src = cov.length ? cov : at;
          for (let c = 0; c < 3; c++) { let s = 0; for (const i of src) s += cur.data[i * 4 + c]; d[(y * nw + x) * 4 + c] = Math.round(s / src.length); }
          d[(y * nw + x) * 4 + 3] = cov.length * 2 >= at.length ? 255 : 0;
          continue;
        }
...
function addRgbTexture(pkg, refs, name, img, gain, opts) {
  ...
  const texRef = pkg.addExport({
    ...
    serialize: (p) => {
      ...
      pr.end();
      const chain = mipChain(px, img.width, img.height, opts && opts.coverage, masked);
      w.cidx(chain.length);
      for (const m of chain) {

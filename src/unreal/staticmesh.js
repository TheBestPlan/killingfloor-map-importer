...
function buildKDOP(vertices, indices, triMaterial) {
  // NB: keep BOTH windings of the two-sided-doubled geometry in the collision tree. KF's static-mesh
  // collision is one-sided (blocks from the triangle's front only), so a floor triangle wound facing DOWN
  // does not catch a pawn from above - the doubled UP-facing copy is what holds it up. Deduplicating to one
  // triangle per face dropped whichever copy came first and let pawns fall through down-wound floors.

...
// Installed Unreal Tournament 99 roots - a folder with Maps\ and System\ under it (the classic layout,
// no mod subfolder). KF_UT99 overrides the search: UT99 predates Steam and the GOG/oldunreal builds
// land wherever the installer was pointed.
function installedUT99() {
  const guesses = [
    process.env.KF_UT99,
    steamApp("Unreal Tournament"),
    ...["ProgramFiles(x86)", "ProgramFiles"].map((v) => process.env[v]).filter(Boolean).flatMap((base) =>
      ["Unreal Tournament", "UnrealTournament", "GOG.com/Unreal Tournament", "GOG Galaxy/Games/Unreal Tournament GOTY"]
        .map((rel) => path.join(base, rel))),
  ].filter(Boolean);
  return guesses.filter((p, i) => guesses.indexOf(p) === i && exists(path.join(p, "Maps")) && exists(path.join(p, "System")));
}
...
module.exports = { wadDirs, skyRoots, installedHalfLife, installedQuake3, installedTacticalOps, installedUT99, clientRoots, modFile, steamApp };

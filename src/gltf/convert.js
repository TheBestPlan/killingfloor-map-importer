...
function convert(opts) {
  ...
  // bTwoSided does not work on KF static meshes, so "two-sided" is done by doubling the geometry
  // (reversed copy of every face) with a single-sided material. This is the fix for see-through walls.
  const doubleGeom = o.twoSided !== false && o.doubleSided !== false;

  // Optional coverage-preserving decimation to a triangle budget (o.maxTris), for maps whose source is
  // far past what KF renders smoothly (CS2, a GTA district). Doubling below multiplies by 2, so aim for
  // half. Clusters vertices onto a shared grid - no dropped meshes, so nothing holes out (src/gltf/decimate).
  if (o.maxTris && scene.prims && scene.prims.length) {
    const target = doubleGeom ? Math.floor(o.maxTris / 2) : o.maxTris;
    const before = scene.prims.reduce((s, p) => s + (p.indices ? p.indices.length / 3 : 0), 0);
    if (before > target) {
      const { decimate } = require("./decimate");
      const r = decimate(scene.prims, target, (o.cell || process.env.KF_CELL) ? { cell: +(o.cell || process.env.KF_CELL) } : undefined);
      scene.prims = r.prims;
      log("decimate: " + Math.round(r.before) + " -> " + Math.round(r.after) + " triangles (coverage kept" + (doubleGeom ? ", pre-double" : "") + ")");
    }
  }

  const meshBuild = buildMeshes(scene, {
    scale, applyMat4: scene.applyMat4, applyMat3: scene.applyMat3, crop, axes: o.axes, flip: o.flip,
    autoColor: !!o.autoColor, groundUp: o.groundUp === true, doubleSided: doubleGeom,
  ...
  for (const key of usedKeys) {
    ...
    // When the geometry is doubled (doubleGeom), the material MUST stay single-sided or the two coincident
    // copies both draw and z-fight. Only fall back to the (unreliable) bTwoSided material flag when not
    // doubling - e.g. a single-model route that opted out.
    const two = doubleGeom ? false : !!(o.twoSided || isTwoSided(mat));
  ...
  holder.killZ = process.env.KF_KILLZ !== undefined ? +process.env.KF_KILLZ : wlo[2] - 2000;
  ...
  if (o.emitPlayerStarts !== false) {
    ...
    if (at) places = [{ loc: at.slice(0, 3), yaw: at[3] !== undefined ? Math.round((-at[3] / 360) * 65536) & 0xffff : 0 }];
      ...
      // Cast down at the centroid; if that column is a gap, sample a grid and take the surface nearest the
      // centroid. topSurfaceAt returns a walkable floor (lowest near-horizontal WITH headroom), so the
      // player lands on the ground floor, not the roof and not inside a wall.
      ...
      if (o.spawnClearance) {
        // topSurfaceAt only guarantees vertical headroom; a wall passing through the spawn column still
        // embeds the pawn (KF kills an encroached pawn - the IGI "spawn in a texture" death). Sample a
        // grid, and among floors, prefer the one nearest the centroid whose body-height band is clear of
        // walls horizontally. Gated so maps that already spawn fine are untouched.
        const N = 20, cands = [];
        for (let i = 0; i <= N; i++) for (let j = 0; j <= N; j++) {
          const x = box.min[0] + (box.max[0] - box.min[0]) * i / N, y = box.min[1] + (box.max[1] - box.min[1]) * j / N;
          const z = topSurfaceAt(meshBuild.meshes, x, y); if (z === null) continue;
          cands.push({ x, y, z, walls: wallsInColumn(meshBuild.meshes, x, y, z + 10, z + 95, 45), d2: (x - gx) ** 2 + (y - gy) ** 2 });
        }
        // genuinely open spots (no wall through the pawn body), nearest the centroid; else least-walled
        const open = cands.filter((c) => c.walls === 0).sort((a, b) => a.d2 - b.d2);
        pick = open[0] || cands.slice().sort((a, b) => a.walls - b.walls || a.d2 - b.d2)[0] || null;
      } else {
        consider(gx, gy);
      ...
      log("spawn: " + (pick ? "on surface at (" + Math.round(pick.x) + "," + Math.round(pick.y) + "," + Math.round(pick.z) + ")" + (o.spawnClearance ? " walls " + pick.walls : "") : "over centre (no surface found)"));
    }
...
// A STANDABLE surface Z at world (x,y): a roughly-horizontal triangle with clear headroom above it, so
// the synthetic spawn lands on a walkable FLOOR, not the roof. Taking the highest surface (the old
// behaviour) put the player inside the roof of any enclosed map (a nightclub, a building) - embedded in
// geometry with no room to stand. Instead collect every surface the column crosses, then pick the lowest
// near-horizontal one whose next surface above is far enough up (HEADROOM) to be open air. Outdoor maps
// still resolve to the ground (its "ceiling" is the sky). Returns null if nothing covers the column.
const HEADROOM = 140;   // KF pawn is ~88 units tall; needs open space above the floor to spawn clear
function topSurfaceAt(meshes, x, y) {
  const hits = [];      // { z, horizontal }
  for (const m of meshes) {
    ...
    for (let t = 0; t + 2 < I.length; t += 3) {
      ...
      const az = A[2] + o[2], bz = B[2] + o[2], cz = C[2] + o[2];
      const z = wa * az + wb * bz + wc * cz;
      // triangle normal Z component (unnormalised is fine for the ratio test)
      const ux = bx - ax, uy = by - ay, uz = bz - az, vx = cx - ax, vy = cy - ay, vz = cz - az;
      const nz = ux * vy - uy * vx, nlen = Math.hypot(uy * vz - uz * vy, uz * vx - ux * vz, nz) || 1;
      hits.push({ z, horizontal: Math.abs(nz / nlen) > 0.5 });
    }
  }
  if (!hits.length) return null;
  hits.sort((p, q) => p.z - q.z);
  for (const h of hits) {                       // lowest horizontal floor with open air above it
    if (!h.horizontal) continue;
    const above = hits.find((o) => o.z > h.z + 20);
    if (!above || above.z - h.z >= HEADROOM) return h.z;
  }
  return hits[hits.length - 1].z;               // no clear floor found: fall back to the highest surface
}

// Spatial hash of world-space vertices (bucketed by `cell`), and nearest-vertex distance to a point - a
// cheap "is this point clear of geometry" test for spawn placement. Only the ±1 neighbour cells are
// searched, so anything farther than ~cell reads as "clear" (Infinity), which is all the spawn test needs.
function buildVertHash(meshes, cell) {
  const h = new Map();
  for (const m of meshes) {
    const o = m.origin;
    for (const v of m.vertices) {
      const x = v.pos[0] + o[0], y = v.pos[1] + o[1], z = v.pos[2] + o[2];
      const k = Math.floor(x / cell) + "," + Math.floor(y / cell) + "," + Math.floor(z / cell);
      let a = h.get(k); if (!a) h.set(k, a = []); a.push(x, y, z);
    }
  }
  return h;
}
function nearestVert(h, cell, x, y, z, ring) {
  ring = ring || 1;
  const cx = Math.floor(x / cell), cy = Math.floor(y / cell), cz = Math.floor(z / cell);
  let best = Infinity;
  for (let dx = -ring; dx <= ring; dx++) for (let dy = -ring; dy <= ring; dy++) for (let dz = -ring; dz <= ring; dz++) {
    const a = h.get((cx + dx) + "," + (cy + dy) + "," + (cz + dz)); if (!a) continue;
    for (let i = 0; i < a.length; i += 3) { const ex = a[i] - x, ey = a[i + 1] - y, ez = a[i + 2] - z; const d = ex * ex + ey * ey + ez * ez; if (d < best) best = d; }
  }
  return Math.sqrt(best);
}

// Count near-VERTICAL triangles (walls) whose bounding box overlaps the pawn's body cylinder at (x,y),
// Z band [zLo,zHi], radius R. Zero means the pawn's body is clear of walls there - a genuinely open spawn
// spot, not merely "no vertices near" (a big wall face has its verts far but still passes through the
// pawn, which is the IGI "spawn inside a mesh -> instant encroachment death"). Horizontal tris (floors)
// are ignored: a floor underfoot is wanted, only walls through the body embed the pawn.
function wallsInColumn(meshes, x, y, zLo, zHi, R) {
  let count = 0;
  for (const m of meshes) {
    const o = m.origin, V = m.vertices, I = m.indices;
    for (let t = 0; t + 2 < I.length; t += 3) {
      ...
      const cx = C[0] + o[0], cy = C[1] + o[1], cz = C[2] + o[2];
      if (Math.max(az, bz, cz) < zLo || Math.min(az, bz, cz) > zHi) continue;
      if (Math.max(ax, bx, cx) < x - R || Math.min(ax, bx, cx) > x + R) continue;
      if (Math.max(ay, by, cy) < y - R || Math.min(ay, by, cy) > y + R) continue;
      const ux = bx - ax, uy = by - ay, uz = bz - az, vx = cx - ax, vy = cy - ay, vz = cz - az;
      const nz = ux * vy - uy * vx, nl = Math.hypot(uy * vz - uz * vy, uz * vx - ux * vz, nz) || 1;
      if (Math.abs(nz / nl) < 0.5) count++;              // vertical-ish face crossing the pawn body = wall
    }
  }
  return count;
}

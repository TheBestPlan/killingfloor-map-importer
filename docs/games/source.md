...
Accepted approximations — each is a known ceiling, not a bug, kept deliberately (the alternatives cost
a lot and cannot be verified against the tested maps without eyeballing every surface):

- **Prop collision** is the render mesh's own kDOP, not its Source VPHYSICS hull, so a highly detailed
  solid mesh (a railed staircase) collides bump-for-bump rather than as the smooth hull Source uses.
  KF authors collision from the render mesh; carrying the separate `.phy` hull is a large addition for a
  feel-only gain.
- **Displacement edge-neighbour stitching** is not done — adjacent displacements of *different power* can
  leave a hairline seam. Removing it needs Source's displacement neighbour graph and is only verifiable
  by eye.
- **Prop LODs** with per-LOD vertex fixups stay on **LOD0** — correct (full detail), just heavier than
  picking a lower LOD for distant props.

Done since: a prop over the 16-bit **vertex** limit is now **split** into parts rather than skipped
(`splitLargeProp`, `test/source.test.js`), so no geometry is dropped.
## CS2 (Source 2)

CS2 is **not** VBSP — it is Source 2, a `.vpk` (v2) whose world is baked into compiled `.vmdl_c`
resources (KV3 + meshopt). It is now parsed **natively** by its own route — see
[`source2.md`](./source2.md):

```
node src/cli.js --game source2 "…/csgo/maps/de_dust2.vpk" --out "…/KillingFloor/Maps" --verify
```

`--game source2 <map>.glb` (already decompiled) still falls through to the [model route](./model.md).

...
### 5.30h A `{` texture is a cut-out only where the ENTITY says so
GoldSrc applies a `{` texture's colour key in one place: a brush entity at `rendermode 4` (Solid). On
worldspawn, and on an entity the mapper left at rendermode 0, the engine draws the whole picture -
key colour and all. `{ladder3b` is 32x32, its palette index 255 is pure blue, 66% of its texels sit
on that index, and all 19 of its faces on fy_dinoiceworld are worldspawn. In Counter-Strike that
ladder is a set of rungs on a flat blue board.

Cut out here instead, two of the map's four ladders became a window onto the skybox. Measured with a
3x3 ray grid from each side of each one: behind `*7` there is a wall at 125 units and behind `*8` at
11 and 188, but behind `*6` and `*9` nine rays out of nine land on a sky brush - the mapper never
built the back of those two alcoves, and the nearest wall face there points away from the player, so
GoldSrc does not draw it either. Nothing in the conversion put the hole there; the opaque ladder is
what covered it.

So a `{` face without an entity asking for a cut-out is drawn solid, the way the engine draws it,
from a copy of the texture with the key colour **bled away** first: index 255 takes the average of
its visible neighbours a few passes deep, so the board comes out the colour of the rungs instead of
blue. Faces affected across the ten shipped maps: fy_dinoiceworld 19, gg_dusty_fortress 48,
gg_33_mario 36, de_winter_austria 28. Every `{` face a mapper meant to see through is on a func_wall
or func_illusionary at `rendermode 4 renderamt 255` and is untouched - de_winter_austria still ships
17 lightmap materials behind a blending Shader, which is its foliage.

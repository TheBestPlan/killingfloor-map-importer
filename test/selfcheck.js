...
console.log("\nBrush entities that draw nothing, and breakables nothing can shoot");
{
  ...
  // A `{` texture is a cut-out only where the ENTITY asks for one. fy_dinoiceworld's ladder sits on
  // worldspawn, so GoldSrc draws its colour key; cut out here, two of its four ladders became a
  // window onto the skybox, because the alcove behind them has no back wall.
  ok("rendermode 4 is what cuts a { texture out",
    be.cutsOut({ classname: "func_wall", rendermode: "4", renderamt: "255" }));
  ok("...and worldspawn, or an entity left at rendermode 0, draws the key colour instead",
    !be.cutsOut(null) && !be.cutsOut({ classname: "func_ladder" }) &&
    !be.cutsOut({ classname: "func_illusionary", rendermode: "3" }));
